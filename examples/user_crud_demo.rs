//! 用户 CRUD 操作演示
//!
//! 这个示例展示了如何使用 UserRepo 仓储接口（repos/user.rs 定义契约，data/user_repo.rs 提供 MySQL 实现）
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 在实际使用中调用: repo.create(&mut new_user).await?;
    ...
    // 2. 根据ID获取用户
    println!("2. 根据ID获取用户:");
    // 在实际使用中调用: let user = repo.get_by_id(new_user.id).await?;
    println!("   可以获取用户ID为 {} 的用户信息\n", new_user.id);

    // 3. 根据手机号获取用户
    println!("3. 根据手机号获取用户:");
    // 在实际使用中调用: let user = repo.get_by_phone("13800138000").await?;
    ...
    // 在实际使用中调用: repo.update(&updated_user).await?;
    ...
    // 6. 获取用户列表
    println!("6. 获取用户列表:");
    // 在实际使用中调用: let users = repo.list(1, 10).await?;
    println!("   可以获取第1页，每页10条的用户列表\n");

    // 7. 搜索用户
    println!("7. 搜索用户:");
    // 在实际使用中调用: let users = repo.search_by_nickname("测试", 1, 10).await?;
    println!("   可以搜索昵称包含'测试'的用户\n");

    // 8. 获取用户总数
    println!("8. 获取用户总数:");
    // 在实际使用中调用: let count = repo.count().await?;
    println!("   可以获取未删除用户的总数\n");

    // 9. 软删除用户
    println!("9. 软删除用户:");
    // 在实际使用中调用: repo.delete(new_user.id, current_timestamp()?).await?;
    println!("   软删除用户ID为 {} 的用户\n", new_user.id);

    // 10. 硬删除用户
    println!("10. 硬删除用户:");
    // 在实际使用中调用: repo.hard_delete(new_user.id).await?;
